...
## Capacitor Android 앱 + Capgo OTA (2026-06-21)
...
- **명령** (`apps/web`): `build:app`(=`vite build --mode capacitor` → `mode==='capacitor'`이면 PWA SW 비활성[vite.config] + `.env.capacitor`의 Railway URL) / `cap:sync`(빌드+동기화, 친구용 APK 빌드 전 필수) / `cap:open`(Android Studio — **APK 빌드는 GUI로**, 시스템 Java 깨져 gradlew CLI 불가) / `ota`(build+Capgo 업로드+채널갱신)
- **배포**: `git push` → Vercel(웹)+Railway(서버) 자동, **앱은 `pnpm --filter @gostop/web ota` 별도**
- **Capgo OTA**: 웹 에셋 변경은 재설치 없이 자동 갱신(`autoUpdate:true`[capacitor.config], `notifyAppReady()` in main.tsx, 채널 `production`)
  - ⚠️ OTA 번들 version은 네이티브 `versionName`(android/app/build.gradle, 현재 `1.0`)보다 높아야 적용 → `apps/web/package.json` version `1.0.1`+, 수정마다 끝자리 +1 후 `ota` (안 올리면 "새 버전 없음"으로 미적용)
  - 네이티브 변경(플러그인/권한/매니페스트)은 OTA 불가 → APK 재빌드. API 키는 `~/.capgo`(login) 또는 `CAPGO_TOKEN` env만, 코드/앱/서버엔 X
