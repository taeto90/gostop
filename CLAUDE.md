...
- **pendingGoStop turn 이동 차단**: server `turnFlow.playCardForPlayer`가 winScore 도달 시 turn 이동 X + `room.pendingGoStop` set. 같은 player가 `declare-go`(다음 turn) / `declare-stop`(phase='ended') 결정해야 진행. AI는 `autoDecideGoStopForAI`에서 2s 후 자동 (hand≥4 & goCount<2면 GO, 그 외 STOP)
- **game 로그 dev only**: `apps/server/src/socket/gameLog.ts`는 `NODE_ENV !== 'production'`일 때만 활성. production은 모든 함수 no-op. `apps/server/logs/game-{roomId}-{instanceId}-{ts}.json`에 매 액션 + 검증 결과(orderValid/cardCheck/stealValid). `.gitignore`에 `logs/` 포함
...
- **DB**: Supabase Postgres (`game_history` table + RLS, anon key). `apps/web/src/lib/supabase.ts`가 URL/KEY 없으면 null 반환 (dev 환경 grace). `gameHistoryStore`에 `toRow` / `fromRow` / `mergeEntries` / `syncFromCloud(myUserId)` 추가 — 로컬 50판 + 클라우드 무제한
...
⚠️ 모션 duration은 `apps/web/src/lib/animationTiming.ts`에서 모두 ×2 처리됨 (HAND_PEAK 0.6s, FLY 0.6s, FLIP 0.6s, SCALE_PEAK 1.0s, INTER_PHASE 0.4s, FLY_TO_COLLECTED 1.0s, COLLECT_STAGGER 0.3s). 사용자 요청에 따라 가시성 ↑ 목적.
...
- ✅ **Go/Stop 시스템** — `pendingGoStop` server state + `GoStopModal`. 본인 turn 끝 winScore 도달 → turn 이동 X + 모달. AI는 `hand≥4 & goCount<2`면 자동 GO, 그 외 STOP
...
- ✅ **테스트모드 production hide** — client UI는 `import.meta.env.DEV` gate, server는 NODE_ENV='production'이면 testMode payload 무시
...
- 🟡 **남은 작업**: 모바일 채팅 키보드 가림 / AI go-stop 정책 개선 / 광팔이 모달 폴리시 / Sentry / README / OAuth / 등급분류 — [`docs/ROADMAP66.md`](docs/ROADMAP66.md) 참고
