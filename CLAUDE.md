...
## Capacitor Android 앱 + DIY OTA (2026-06-21)

`apps/web`를 Capacitor로 Android 앱화 (iOS 제외). appId `com.gostop.app`, 이름 `고스톱`.
네이티브 = `apps/web/android/` (Glob엔 안 잡힘=gitignore, Read는 절대경로로 가능).

- **명령** (`apps/web`): `build:app`(=`vite build --mode capacitor` → `mode==='capacitor'`이면 PWA SW 비활성[vite.config] + `.env.capacitor`의 Railway URL) / `cap:sync`(빌드+동기화, 친구용 APK 빌드 전 필수) / `cap:open`(Android Studio — **APK 빌드는 GUI로**, 시스템 Java 깨져 gradlew CLI 불가) / `ota:diy`(build+zip+Supabase Storage 업로드)
- **배포**: `git push` → Vercel(웹)+Railway(서버) 자동, **앱은 `pnpm --filter @gostop/web ota:diy` 별도**
- **DIY OTA** (Capgo Cloud 미사용 — 무료): `@capgo/capacitor-updater` 플러그인을 **수동 모드**(`autoUpdate:false`[capacitor.config])로 사용. `lib/otaUpdate.ts`가 앱 시작 시 Supabase Storage public 버킷 `ota`의 `latest.json` 확인 → 더 새 version이면 `download`+`set`(다음 실행 적용) + `notifyAppReady()`(롤백 안전장치). 업로드는 `scripts/ota-diy.mjs`(dist→zip→Storage upsert). 필요 env: `apps/web/.env`에 `SUPABASE_SERVICE_ROLE_KEY`(VITE_ 접두사 X라 앱 번들 미노출)
  - ⚠️ OTA 번들 version은 현재 실행 중 번들(`package.json` version, 빌드 시 baked)보다 높아야 적용 → 수정마다 `apps/web/package.json` version 끝자리 +1 후 `ota:diy`
  - 네이티브 변경(플러그인/권한/매니페스트/`autoUpdate` 등 capacitor.config)은 OTA 불가 → APK 재빌드·재설치. **Capgo Cloud(autoUpdate)→DIY 전환도 네이티브 변경이라 일회성 APK 재빌드 필요**
- **OAuth(앱)**: `signInWithOAuth({redirectTo:'com.gostop.app://login-callback', skipBrowserRedirect})` + `@capacitor/browser` + authStore `appUrlOpen` deep-link 핸들러(implicit 토큰 재사용, LoginPage/authStore는 `Capacitor.isNativePlatform()`로 분기). Google 콘솔 변경 X(Supabase 콜백 경유), **Supabase Redirect URLs + 서버 CORS_ORIGIN(Railway)에 앱 origin `https://localhost`/scheme 등록 필요**
- **네이티브**: AndroidManifest(CAMERA/RECORD_AUDIO/MODIFY_AUDIO_SETTINGS + custom scheme intent-filter + `screenOrientation="sensorLandscape"`), MainActivity=몰입모드(상태바·내비바 숨김), styles.xml cutout `shortEdges` + `#root` safe-area-inset, 뒤로가기=`NativeBackButtonGuard`(종료 확인 모달)
- **LiveKit 음성품질**: `LiveKitGameRoom`의 `options`(AudioPresets.musicHighQuality+red+노이즈억제). 기본 voiceOnly라 카메라는 대기실 방설정 mediaMode='video'로 변경해야 가능
- **버전 표시**: `LobbyProfileCard`가 package.json version import → 로그아웃 위 `v{version}` (OTA 갱신 시 새 버전 표시)
...
- ✅ **마지막 손패 즉시승** (2026-06-21) — turnFlow.ts `wonOnLastCard`: 손패 0 + winScore 도달 시 즉시 종료(후턴 진행 X, go/stop 없이 본인 승리)
- ✅ **나가리 무승부** (2026-06-21) — ResultView가 `decideWinnerUserId`(result/helpers)로 아무도 winScore 미달이면 우승자 없이 "나가리 무승부" 표시 (점수 최고여도 미달이면 무승부)
