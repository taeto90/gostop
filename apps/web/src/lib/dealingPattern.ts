...
import { applySpeed } from './animationTiming.ts';

const ROUND_GAP = 0.45; // 라운드 사이 wait
const PER_CARD = 0.05;
const gap = () => applySpeed(ROUND_GAP);
const per = () => applySpeed(PER_CARD);
...
export function handDealDelay(i: number, total: number): number {
  if (total === 7) {
    ...
    if (i < 4) return i * per();
    return gap() * 2 + (i - 4) * per();
  }
  if (total === 10) {
    // 2인: 첫 5장 → 바닥 4장 → 다음 5장
    if (i < 5) return i * per();
    return gap() * 2 + (i - 5) * per();
  }
  // 기타: 그냥 stagger
  return i * per();
}
...
export function fieldDealDelay(i: number, total: number): number {
  if (total === 6) {
    ...
    if (i < 3) return gap() + i * per();
    return gap() * 3 + (i - 3) * per();
  }
  if (total === 8) {
    // 2인: 라운드 1 끝나고 4장 → 라운드 2 끝나고 4장
    if (i < 4) return gap() + i * per();
    return gap() * 3 + (i - 4) * per();
  }
  return i * per();
}
...
export function totalDealDuration(handTotal: number): number {
  return handDealDelay(handTotal - 1, handTotal) + applySpeed(0.2);
}
